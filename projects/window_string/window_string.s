...
INCLUDE "smiley.inc"
INCLUDE "font.inc"
INCLUDE "window_string.inc"

; Defines
VRAM_OBJ_DATA_START     EQU $8000
VRAM_OBJ_DATA_END       EQU $8FFF
...
VRAM_WINDOW_MAP_START   EQU $9C00
VRAM_WINDOW_MAP_END     EQU $9FFF
VRAM_WINDOW_MAP_LEN     EQU (VRAM_WINDOW_MAP_END - VRAM_WINDOW_MAP_START) + 1
...
LCDC                EQU $FF40
LCDC_BG_FLAG        EQU (1 << 0)    ; 0 = Off, 1 = On
...
FONT_CODE_OFFSET        EQU 1

STRING_TO_WRITE         EQUS    "HELLO,WORLD"
;STRING_TO_WRITE_LEN     EQU     {STRING_TO_WRITE}.length
StringToWrite:          DB      {STRING_TO_WRITE}
STRING_TO_WRITE_LEN     EQU     STRLEN({STRING_TO_WRITE}) 

WAIT_FOR_VBLANK: MACRO
.wait_for_vblank\@
    ldh     a, [STAT]
    ;and     2              ; Actually checks for H-Blank or V-Blank
    and     3               ; Checks if A is zero
    jr      nz, .wait_for_vblank\@
ENDM

CLEAR_VRAM: MACRO
    ld      hl, VRAM_BG_MAP_START
    ld      bc, VRAM_BG_MAP_LEN
    ld      a, 0
.clear_vram_loop\@
    ld      [hl], 1             ; Using 1 because it's a blank tile
    inc     hl
    dec     bc
    cp      b
...
; Adjusts ASCII character code based on where font tile data is stored
APPLY_FONT_OFFSET: MACRO
IF FONT_CODE_OFFSET < $20
    sub     a, ($20 - FONT_CODE_OFFSET)
ELSE
    add     a, (FONT_CODE_OFFSET - $20)
ENDC
ENDM

; Waits for VBlank then copies string data to VRAM
; Register Parameters:
;   DE - source address
;   HL - VRAM destination address
;   B  - length
WriteStringToVRAM:
    nop                         ; A race condition is causing LCDC to lock out CPU after passing V-Blank check
    nop                         ; I have to find a better way to handle this
    nop
    nop
.loop
    WAIT_FOR_VBLANK
...
main:
    ...
    ld      hl, BG_PAL0
    ld      [hl], BG_PAL0_DEFAULT

    ; Load font into OBJ memory
    ld      hl, VRAM_OBJ_DATA_START + (VRAM_OBJ_LEN * FONT_CODE_OFFSET)
    ld      de, Font
    ld      c, FontLen
.load_font_loop
    ld      b, VRAM_OBJ_LEN             ; Must reload with each iteration because WriteVRAM 
                                        ; decrements it to zero
    call    WriteVRAM
    dec     c
    jp      nz, .load_font_loop

    ; Enable LCDC and OBJ sprites
    ld      hl, LCDC
    ld      [hl], LCDC_BG_FLAG | LCDC_OBJ_FLAG | LCDC_CTRL_STOP_FLAG | LCDC_BG_DATA_FLAG    ; Enable BG, OBJ sprites, and LCD controller

    ; Write string character codes to BG
    ld      hl, VRAM_BG_MAP_START       ; Write string starting at tile 0
    ld      b, STRING_TO_WRITE_LEN
    ld      de, StringToWrite
    call    WriteStringToVRAM
